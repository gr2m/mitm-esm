 ...
 */
export default function InternalSocket(remote) {
...
InternalSocket.prototype.getAsyncId = function () {
  return this.id;
};
...
InternalSocket.prototype.writev = function (_req, data) {
  for (let i = 0; i < data.length; ++i) this._write(data[i], data[++i], noop);
  return NO_ERROR;
};
...
export function createInternalSocketPair() {
  const a = Object.create(InternalSocket.prototype);
  const b = Object.create(InternalSocket.prototype);
