...
describe("Mitm", function () {
  ...
  function mustConnect(module) {
    describe("as connect", function () {
      ...
      it("must return an instance of Net.Socket given port and host", function () {
      ...
      it("must emit connect on Mitm with options object given host and port", function () {
        var onConnect = Sinon.spy();
        this.mitm.on("connect", onConnect);
      ...
      // object.
      it("must call back on connect given port, host and callback", function (done) {
      ...
      describe("when bypassed", function () {
        ...
        it("must not intercept", function (done) {
          this.mitm.on("connect", function (client) {
            client.bypass();
          });
        ...
        it("must not call back twice on connect given callback", function (done) {
          ...
          client.on(
            "error",
            process.nextTick.bind(null, function () {
  ...
  describe("Net.connect", function () {
    ...
    describe("Socket", function () {
      describe(".prototype.write", function () {
        ...
        it("must write to server from client given a UTF-8 string", function (done) {
          var server;
          this.mitm.on("connection", function (s) {
            server = s;
          });
        ...
        it("must write to server from client given a ASCII string", function (done) {
          var server;
          this.mitm.on("connection", function (s) {
            server = s;
          });
        ...
        it("must write to server from client given a UCS-2 string", function (done) {
          var server;
          this.mitm.on("connection", function (s) {
            server = s;
          });
